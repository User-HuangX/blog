...
#
# 环境变量：
#   DEPLOY_HOST          远程主机（部署到远程时必须设置）
#   DEPLOY_USER          SSH 用户（部署到远程时必须设置）
...
# 远程部署必须有 DEPLOY_HOST 和 DEPLOY_USER
if [[ -z "$DEPLOY_HOST" || -z "$DEPLOY_USER" ]]; then
    echo "错误: 远程部署必须设置 DEPLOY_HOST 和 DEPLOY_USER 环境变量。" >&2
    echo "例如: DEPLOY_HOST=your-server.com DEPLOY_USER=ubuntu ./build-images.sh" >&2
    echo "或本地构建: DEPLOY_LOCAL=1 ./build-images.sh" >&2
    exit 1
fi
